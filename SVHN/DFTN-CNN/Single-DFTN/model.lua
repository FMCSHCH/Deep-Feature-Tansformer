...
cmd:option('-batchSize', 35, 'mini-batch size (1 = pure stochastic)')
...
cmd:option('-tra', false, 'translation')
...
function networks.new_spatial_tranformer_new2(locnet, rot, sca, tra,
                                         input_size, input_channels,
                                         no_cuda)
...
print (init_bias)
  ...
  local conv1 = networks.new_conv2(128, 64,
	...
	-- new_network
		...
		local new_network=nn.ConcatTable()
		local branch_1_new_network=nn.Sequential()
	    VGG_model = torch.load('VGG_inside_ST2.bin')
		branch_1_new_network:add(VGG_model)
		branch_1_new_network:add(CVPR_network) 
		conv_128_512=networks.new_conv2(128, 48,
                                           	 false, false, 3)	
	    branch_1_new_network:add(conv_128_512)
...
function networks.new_spatial_tranformer_new3(locnet, rot, sca, tra,
                                         input_size, input_channels,
                                         no_cuda)
...
print (init_bias)
  ...
  local conv1 = networks.new_conv2(128, 64,
	...
	-- new_network
		...
		local new_network=nn.ConcatTable()
		local branch_1_new_network=nn.Sequential()
	    VGG_model = torch.load('VGG_inside_ST3.bin')
	    branch_1_new_network:add(VGG_model)
		branch_1_new_network:add(CVPR_network) 
		conv_128_512=networks.new_conv2(128, 64,
                                           	 false, false, 3)	
	    branch_1_new_network:add(conv_128_512)
...
function networks.new_spatial_tranformer_new4(locnet, rot, sca, tra,
                                         input_size, input_channels,
                                         no_cuda)
...
print (init_bias)
  ...
  local conv1 = networks.new_conv2(128, 64,
	...
	-- new_network
		...
		local new_network=nn.ConcatTable()
		local branch_1_new_network=nn.Sequential()
	    VGG_model = torch.load('VGG_inside_ST4.bin')
	    branch_1_new_network:add(VGG_model)
		branch_1_new_network:add(CVPR_network) 
		conv_128_512=networks.new_conv2(128, 128,
                                           	 false, false, 3)	
	    branch_1_new_network:add(conv_128_512)
...
--[[MODEL]]--
local conv1 = networks.new_conv2(512, 512,false, false, 3)
local conv2 = networks.new_conv2(512, 192,false, false, 3)
local conv3 = networks.new_conv2(192, 192,false, false, 3)
model = nn.Sequential()
--percep_model = torch.load('VGG_Network.t7')
--Trained_model = torch.load('Trained_Network_1.bin')
--L1=Trained_model:get(1)
--L2=Trained_model:get(2)
--model:add(L1)
--model:add(L2)
percep_model = torch.load('First_VGG.bin')
model:add(percep_model)
First_ST=torch.load('First_ST.bin')
--model:add(networks.new_spatial_tranformer_new(opt.locnet,false, false, false,nil, nil,opt.no_cuda))
model:add(First_ST)
--model:add(networks.new_spatial_tranformer_new(opt.locnet,false, false, false,nil, nil,opt.no_cuda))
model:add(convLayer(512, 256, 2))
model:add(convLayer(256, 128, 2))
model:add(convLayer(128, 192, 2))
...
--Adding ST4
--model:add(networks.new_spatial_tranformer_new(opt.locnet,false, false, false,nil, nil,opt.no_cuda))
model:add(convLayer(512, 160, 2))
model:add(convLayer(160, 192, 2))
model:add(convLayer(192, 192, 1))
model:add(convLayer(192, 192, 2))
model:add(convLayer(192, 192, 1))
model:add(convLayer(192, 192, 2))
model:add(convLayer(192, 192, 2))
]]
model:add(nn.View(192*8*8):setNumInputDims(3))
model:add(nn.Linear(192*8*8, 3072))
...
model:add(sequencer)	
print(model)
--torch.save('model_sample.bin',model)
--[[LOSS]]--
...
-- loading datasets
trainSetPath = '/data/hossamkasem/5-SVHN/6-Ours_Single/New_train_daset.bin'
...
print(trainSet)
testSetPath = '/data/hossamkasem/5-SVHN/6-Ours_Single/test_dataset.bin'
...
local function train(epochs)
  ...
  torch.save('/data/hossamkasem/5-SVHN/6-Ours_Single/results/train_confusion_matrix/Train_confusion_'..epochs..'.bin', accuracy)
  print('Train Error = ' .. TrainError .. '\n')  
end
...
local function test(epochs)
  ...
  torch.save('/data/hossamkasem/5-SVHN/6-Ours_Single/results/test_confusion_matrix/Test_confusion_'..epochs..'.bin', accuracy)
...
for e = 1,opt.epochs do
  ...
  torch.save('/data/hossamkasem/5-SVHN/6-Ours_Single/results/Trained_Network_'..epochs..'.bin', model)
  end
...
--[[
t=torch.load('train_dataset.bin')
temp_train_data=torch.zeros(130000,1,54,54)
temp_train_label=torch.zeros(130000,7)
for i=1,130000 do
temp_train_data[i]=t.data[i]
temp_train_label[i]=t.label[i]
end
tt={}
tt.data=temp_train_data:float()
tt.label=temp_train_label:float()
torch.sava('New_train_daset.bin',tt)
]]