...
cmd:option('-load', '/data/hossamkasem/5-SVHN/4-Mutipule_ST_CNN/Trained_Network_28.bin', 'load saved model as starting point')
...
cmd:option('-locnet2', "48,60,30", 'Localization network')
cmd:option('-locnet3', "64,128,64", 'Localization network')
cmd:option('-locnet4', "128,256,128", 'Localization network')
...
function networks.new_spatial_tranformer2(locnet, rot, sca, tra,
                                         input_size, input_channels,
                                         no_cuda)
...
function networks.new_spatial_tranformer3(locnet, rot, sca, tra,
                                         input_size, input_channels,
                                         no_cuda)
...
function networks.new_spatial_tranformer4(locnet, rot, sca, tra,
                                         input_size, input_channels,
                                         no_cuda)
...
l2_2=Trained_model:get(2):get(2)
l2_1=Trained_model:get(2):get(1)
l2_1:insert(networks.new_spatial_tranformer2(opt.locnet2,opt.rot, opt.sca, opt.tra,nil, nil,opt.no_cuda),2)
l2_1:insert(networks.new_spatial_tranformer3(opt.locnet3,opt.rot, opt.sca, opt.tra,nil, nil,opt.no_cuda),4)
l2_1:insert(networks.new_spatial_tranformer4(opt.locnet4,opt.rot, opt.sca, opt.tra,nil, nil,opt.no_cuda),6)
L2=nn.Sequential()
L2:add(l2_1)
L2:add(l2_2)
--[[MODEL]]--
...
-- loading datasets
trainSetPath = '/data/hossamkasem/5-SVHN/4-Mutipule_ST_CNN/train_dataset.bin'
trainSet = torch.checkpoint(trainSetPath) 
trainSize = trainSet.label:size()[1]
print(trainSet)
testSetPath = '/data/hossamkasem/5-SVHN/4-Mutipule_ST_CNN/test_dataset.bin'
...
local function train(epochs)
  ...
  torch.save('/data/hossamkasem/5-SVHN/4-Mutipule_ST_CNN/results/train_confusion_matrix/Train_confusion_'..epochs..'.bin', accuracy)
  print('Train Error = ' .. TrainError .. '\n')  
end
...
local function test(epochs)
  ...
  torch.save('/data/hossamkasem/5-SVHN/4-Mutipule_ST_CNN/results/test_confusion_matrix/Test_confusion_'..epochs..'.bin', accuracy)
...
for e = 1,opt.epochs do
  ...
  torch.save('/data/hossamkasem/5-SVHN/4-Mutipule_ST_CNN/results/Trained_Network_'..epochs..'.bin', model)
  end
