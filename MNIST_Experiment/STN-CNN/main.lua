...
if opt.eval then
  ...
  while opt.e == -1 or opt.e >= epoch do
    ...
    if opt.output ~= '' then
     torch.save('/data/hossamkasem/4-Original_STN_Experminets/2-Networks/3-original_STN_CNN_R_wih_n_20/Saved_network/Trained_Network_'..epoch..'.bin', network)
      torch.save(opt.output.."norm", {mean, std})
	  torch.save('/data/hossamkasem/4-Original_STN_Experminets/2-Networks/3-original_STN_CNN_R_wih_n_20/Saved_network/accuracy_'..epoch..'.bin', accuracy)
    end
    epoch = epoch + 1
  end
end
